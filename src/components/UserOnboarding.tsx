...
const UserOnboarding: React.FC<UserOnboardingProps> = ({ children }) => {
  ...
  useEffect(() => {
    if (!showOverlay && username) {
      ...
      const timer = setTimeout(() => {
        // After fade out, remove overlay content from DOM
  ...
  const handleLetsGo = () => {
    if (inputName.trim()) {
      localStorage.setItem(LOCAL_STORAGE_KEY, inputName.trim());
  ...
  return (
    <div
      className={`bg-opacity-90 fixed inset-0 flex items-center justify-center bg-gray-900 transition-opacity duration-500 ease-in-out ${isFadingOut ? "opacity-0" : "opacity-100"} `}
      style={{ zIndex: 1000 }}
    >
